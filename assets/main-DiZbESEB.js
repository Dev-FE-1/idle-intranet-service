...
`;class fr extends z{constructor(){super("#main"),this.announcement=null,this.Icon=new U({svg:rn,options:{color:Y.DARKEST_GRAY,size:"1.4rem"}})}async setAnnouncement(){const e=window.location.pathname.split("/"),n=e[e.length-1];this.announcement=await nr(n)}async setMember(){this.member=await nn(this.announcement.employeeNumber)}async render(){await this.setAnnouncement(),await this.setMember(),this.$container.innerHTML=`
...
`,"<br />")}
    ...
    `}}class ne{constructor({placeholder:e,type:n,id:s,readOnly:r,name:i,value:o}){this.placeholder=e||"",this.type=n||"text",this.id=s,this.readOnly=r?"read-only":"default",this.name=i||this.id,this.value=o||""}html(){return`
    ...
    `}}var Rr=typeof globalThis<"u"?globalThis:typeof window<"u"?window:typeof global<"u"?global:typeof self<"u"?self:{};function Ir(t){return t&&t.__esModule&&Object.prototype.hasOwnProperty.call(t,"default")?t.default:t}var cn={exports:{}};(function(t,e){(function(n,s){t.exports=s()})(Rr,function(){var n=1e3,s=6e4,r=36e5,i="millisecond",o="second",a="minute",d="hour",u="day",l="week",f="month",M="quarter",$="year",g="date",v="Invalid Date",D=/^(\d{4})[-/]?(\d{1,2})?[-/]?(\d{0,2})[Tt\s]*(\d{1,2})?:?(\d{1,2})?:?(\d{1,2})?[.:]?(\d+)?$/,x=/\[([^\]]+)]|Y{1,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a|A|m{1,2}|s{1,2}|Z{1,2}|SSS/g,G={name:"en",weekdays:"Sunday_Monday_Tuesday_Wednesday_Thursday_Friday_Saturday".split("_"),months:"January_February_March_April_May_June_July_August_September_October_November_December".split("_"),ordinal:function(y){var p=["th","st","nd","rd"],h=y%100;return"["+y+(p[(h-20)%10]||p[h]||p[0])+"]"}},A=function(y,p,h){var w=String(y);return!w||w.length>=p?y:""+Array(p+1-w.length).join(h)+y},ie={s:A,z:function(y){var p=-y.utcOffset(),h=Math.abs(p),w=Math.floor(h/60),m=h%60;return(p<=0?"+":"-")+A(w,2,"0")+":"+A(m,2,"0")},m:function y(p,h){if(p.date()<h.date())return-y(h,p);var w=12*(h.year()-p.year())+(h.month()-p.month()),m=p.clone().add(w,f),E=h-m<0,S=p.clone().add(w+(E?-1:1),f);return+(-(w+(h-m)/(E?m-S:S-m))||0)},a:function(y){return y<0?Math.ceil(y)||0:Math.floor(y)},p:function(y){return{M:f,y:$,w:l,d:u,D:g,h:d,m:a,s:o,ms:i,Q:M}[y]||String(y||"").toLowerCase().replace(/s$/,"")},u:function(y){return y===void 0}},oe="en",N={};N[oe]=G;var Z="$isDayjsObject",me=function(y){return y instanceof ke||!(!y||!y[Z])},Te=function y(p,h,w){var m;if(!p)return oe;if(typeof p=="string"){var E=p.toLowerCase();N[E]&&(m=E),h&&(N[E]=h,m=E);var S=p.split("-");if(!m&&S.length>1)return y(S[0])}else{var R=p.name;N[R]=p,m=R}return!w&&m&&(oe=m),m||!w&&oe},O=function(y,p){if(me(y))return y.clone();var h=typeof p=="object"?p:{};return h.date=y,h.args=arguments,new ke(h)},k=ie;k.l=Te,k.i=me,k.w=function(y,p){return O(y,{locale:p.$L,utc:p.$u,x:p.$x,$offset:p.$offset})};var ke=function(){function y(h){this.$L=Te(h.locale,null,!0),this.parse(h),this.$x=this.$x||h.x||{},this[Z]=!0}var p=y.prototype;return p.parse=function(h){this.$d=function(w){var m=w.date,E=w.utc;if(m===null)return new Date(NaN);if(k.u(m))return new Date;if(m instanceof Date)return new Date(m);if(typeof m=="string"&&!/Z$/i.test(m)){var S=m.match(D);if(S){var R=S[2]-1||0,C=(S[7]||"0").substring(0,3);return E?new Date(Date.UTC(S[1],R,S[3]||1,S[4]||0,S[5]||0,S[6]||0,C)):new Date(S[1],R,S[3]||1,S[4]||0,S[5]||0,S[6]||0,C)}}return new Date(m)}(h),this.init()},p.init=function(){var h=this.$d;this.$y=h.getFullYear(),this.$M=h.getMonth(),this.$D=h.getDate(),this.$W=h.getDay(),this.$H=h.getHours(),this.$m=h.getMinutes(),this.$s=h.getSeconds(),this.$ms=h.getMilliseconds()},p.$utils=function(){return k},p.isValid=function(){return this.$d.toString()!==v},p.isSame=function(h,w){var m=O(h);return this.startOf(w)<=m&&m<=this.endOf(w)},p.isAfter=function(h,w){return O(h)<this.startOf(w)},p.isBefore=function(h,w){return this.endOf(w)<O(h)},p.$g=function(h,w,m){return k.u(h)?this[w]:this.set(m,h)},p.unix=function(){return Math.floor(this.valueOf()/1e3)},p.valueOf=function(){return this.$d.getTime()},p.startOf=function(h,w){var m=this,E=!!k.u(w)||w,S=k.p(h),R=function(ce,F){var Q=k.w(m.$u?Date.UTC(m.$y,F,ce):new Date(m.$y,F,ce),m);return E?Q:Q.endOf(u)},C=function(ce,F){return k.w(m.toDate()[ce].apply(m.toDate("s"),(E?[0,0,0,0]:[23,59,59,999]).slice(F)),m)},P=this.$W,_=this.$M,V=this.$D,pe="set"+(this.$u?"UTC":"");switch(S){case $:return E?R(1,0):R(31,11);case f:return E?R(1,_):R(0,_+1);case l:var ae=this.$locale().weekStart||0,we=(P<ae?P+7:P)-ae;return R(E?V-we:V+(6-we),_);case u:case g:return C(pe+"Hours",0);case d:return C(pe+"Minutes",1);case a:return C(pe+"Seconds",2);case o:return C(pe+"Milliseconds",3);default:return this.clone()}},p.endOf=function(h){return this.startOf(h,!1)},p.$set=function(h,w){var m,E=k.p(h),S="set"+(this.$u?"UTC":""),R=(m={},m[u]=S+"Date",m[g]=S+"Date",m[f]=S+"Month",m[$]=S+"FullYear",m[d]=S+"Hours",m[a]=S+"Minutes",m[o]=S+"Seconds",m[i]=S+"Milliseconds",m)[E],C=E===u?this.$D+(w-this.$W):w;if(E===f||E===$){var P=this.clone().set(g,1);P.$d[R](C),P.init(),this.$d=P.set(g,Math.min(this.$D,P.daysInMonth())).$d}else R&&this.$d[R](C);return this.init(),this},p.set=function(h,w){return this.clone().$set(h,w)},p.get=function(h){return this[k.p(h)]()},p.add=function(h,w){var m,E=this;h=Number(h);var S=k.p(w),R=function(_){var V=O(E);return k.w(V.date(V.date()+Math.round(_*h)),E)};if(S===f)return this.set(f,this.$M+h);if(S===$)return this.set($,this.$y+h);if(S===u)return R(1);if(S===l)return R(7);var C=(m={},m[a]=s,m[d]=r,m[o]=n,m)[S]||1,P=this.$d.getTime()+h*C;return k.w(P,this)},p.subtract=function(h,w){return this.add(-1*h,w)},p.format=function(h){var w=this,m=this.$locale();if(!this.isValid())return m.invalidDate||v;var E=h||"YYYY-MM-DDTHH:mm:ssZ",S=k.z(this),R=this.$H,C=this.$m,P=this.$M,_=m.weekdays,V=m.months,pe=m.meridiem,ae=function(F,Q,ye,Ae){return F&&(F[Q]||F(w,E))||ye[Q].slice(0,Ae)},we=function(F){return k.s(R%12||12,F,"0")},ce=pe||function(F,Q,ye){var Ae=F<12?"AM":"PM";return ye?Ae.toLowerCase():Ae};return E.replace(x,function(F,Q){return Q||function(ye){switch(ye){case"YY":return String(w.$y).slice(-2);case"YYYY":return k.s(w.$y,4,"0");case"M":return P+1;case"MM":return k.s(P+1,2,"0");case"MMM":return ae(m.monthsShort,P,V,3);case"MMMM":return ae(V,P);case"D":return w.$D;case"DD":return k.s(w.$D,2,"0");case"d":return String(w.$W);case"dd":return ae(m.weekdaysMin,w.$W,_,2);case"ddd":return ae(m.weekdaysShort,w.$W,_,3);case"dddd":return _[w.$W];case"H":return String(R);case"HH":return k.s(R,2,"0");case"h":return we(1);case"hh":return we(2);case"a":return ce(R,C,!0);case"A":return ce(R,C,!1);case"m":return String(C);case"mm":return k.s(C,2,"0");case"s":return String(w.$s);case"ss":return k.s(w.$s,2,"0");case"SSS":return k.s(w.$ms,3,"0");case"Z":return S}return null}(F)||S.replace(":","")})},p.utcOffset=function(){return 15*-Math.round(this.$d.getTimezoneOffset()/15)},p.diff=function(h,w,m){var E,S=this,R=k.p(w),C=O(h),P=(C.utcOffset()-this.utcOffset())*s,_=this-C,V=function(){return k.m(S,C)};switch(R){case $:E=V()/12;break;case f:E=V();break;case M:E=V()/3;break;case l:E=(_-P)/6048e5;break;case u:E=(_-P)/864e5;break;case d:E=_/r;break;case a:E=_/s;break;case o:E=_/n;break;default:E=_}return m?E:k.a(E)},p.daysInMonth=function(){return this.endOf(f).$D},p.$locale=function(){return N[this.$L]},p.locale=function(h,w){if(!h)return this.$L;var m=this.clone(),E=Te(h,w,!0);return E&&(m.$L=E),m},p.clone=function(){return k.w(this.$d,this)},p.toDate=function(){return new Date(this.valueOf())},p.toJSON=function(){return this.isValid()?this.toISOString():null},p.toISOString=function(){return this.$d.toISOString()},p.toString=function(){return this.$d.toUTCString()},y}(),dt=ke.prototype;return O.prototype=dt,[["$ms",i],["$s",o],["$m",a],["$H",d],["$W",u],["$M",f],["$y",$],["$D",g]].forEach(function(y){dt[y[1]]=function(p){return this.$g(p,y[0],y[1])}}),O.extend=function(y,p){return y.$i||(y(p,ke,O),y.$i=!0),O},O.locale=Te,O.isDayjs=me,O.unix=function(y){return O(1e3*y)},O.en=N[oe],O.Ls=N,O.p={},O})})(cn);var Mr=cn.exports;const Rt=Ir(Mr);class Cr{constructor(){this.$time=document.querySelector(".work-hour-time.current"),this.timeout=null,this.timer=null}updateTime(){const e=Rt().format("HH:mm");this.$time&&(this.$time.setAttribute("datetime",e),this.$time.innerText=e)}getNextUpdateDelay(){const e=Rt();return e.add(1,"minute").startOf("minute").diff(e)}cleanUp(){this.timeout&&(clearTimeout(this.timeout),this.timeout=null),this.timer&&(clearTimeout(this.timer),this.timer=null)}render(){if(this.updateTime(),!this.timer){const e=this.getNextUpdateDelay(),n=()=>{this.updateTime();const s=this.getNextUpdateDelay();this.timer=setTimeout(n,s)};this.timeout=setTimeout(n,e)}}}var Lr={VITE_SERVER_URL:"https://equal-rosemarie-deun-b63b6790.koyeb.app",VITE_BASE_URL:"/idle-intranet-service",BASE_URL:"/idle-intranet-service/",MODE:"production",DEV:!1,PROD:!0,SSR:!1};const{VITE_SERVER_URL:Or}=Lr,ln=Or,Pr=async t=>{try{const e=await fetch(`${ln}/api/addAttendance`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!e.ok)throw new Error(`Failed to add attendance data: ${e.statusText}`);return await e.json()}catch(e){throw console.error("Error adding attendance data:",e),e}},Dr=async t=>{try{const e=await fetch(`${ln}/api/updateAttendance`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!e.ok)throw new Error(`Failed to update attendance data: ${e.statusText}`);return await e.json()}catch(e){throw console.error("Error updating attendance data:",e),e}};class xr{constructor({personalInfo:e}){this.store=H,this.weeklyAttendances=null,this.weeklyWorkHours=null,this.Icon=new U({svg:sn,options:{color:Y.DARK_GRAY}}),this.personalInfo=e,this.modal=new at({onSubmit:()=>{},id:"work-toggle-modal"})}renderWeeklyWorkHours(){const e=document.querySelector(".weekly-work-hours-time");e&&(e.innerHTML=`${this.weeklyWorkHours}시간`)}renderProgressRing(){const e=document.querySelector(".progress-ring-container"),n=this.weeklyWorkHours/40*100;this.ProgressRing=new Ar({percent:n}),e&&(e.innerHTML=this.ProgressRing.html())}setModal({title:e,mainContent:n,buttonContent:s,onSubmit:r}){const i=document.body.querySelector(".work-modal-wrapper");this.modal.title=e,this.modal.mainContent=n,this.modal.buttonContent=s,this.modal.updateButton(),this.modal.onSubmit=r,i.innerHTML=this.modal.html()}async renderDailyWork(){const e=new Date().toISOString().split("T")[0];let n=this.weeklyAttendances.find(d=>d.date===e);n||(n=tn(e),this.weeklyAttendances.push(n));let{startTime:s,endTime:r}=n;this.Button=new ue({variant:s&&!r?"primary":"secondary",size:"large",content:s&&!r?"근무 종료":"근무 시작",disabled:!!r});const i=document.querySelector(".work-hours-button-container"),o=document.querySelector(".work-hour-time.start"),a=document.querySelector(".work-hour-time.end");i&&(i.innerHTML=this.Button.html()),this.CurrentTime=new Cr,this.CurrentTime.cleanUp(),this.CurrentTime.render(),s&&(o.innerText=s,o.setAttribute("datetime",s)),r&&(a.innerText=r,a.setAttribute("datetime",r)),i.querySelector("button").addEventListener("click",async()=>{const d=document.body.querySelector(".work-hour-time.current").innerHTML;s&&!r?this.setModal({title:`현재 시각: ${d}`,mainContent:"근무를 종료하시겠습니까?",buttonContent:"근무 종료",onSubmit:async()=>{r=d,a.innerText=r||"-",a.setAttribute("datetime",r);const u={employeeNumber:this.user.employeeNumber,date:e,startTime:s,endTime:r,status:"정상 근무"};await Dr(u),n.endTime=r,await this.renderDailyWork(),this.personalInfo.updateIsWorking()}}):s||this.setModal({title:`현재 시각: ${d}`,mainContent:"근무를 시작하시겠습니까?",buttonContent:"근무 시작",onSubmit:async()=>{s=d,o.innerText=s||"-",o.setAttribute("datetime",s);const u={employeeNumber:this.user.employeeNumber,date:e,startTime:s,endTime:null,status:"정상 근무"};await Pr(u),n.startTime=s,await this.renderDailyWork(),this.personalInfo.updateIsWorking()}}),this.modal.render(),this.modal.open()})}async render(){this.weeklyAttendances||(this.user=await this.store.getUser(),this.weeklyAttendances=await this.store.getWeeklyAttendances(),this.weeklyWorkHours=Ys(this.weeklyAttendances)),this.renderWeeklyWorkHours(),this.renderProgressRing(),this.renderDailyWork()}html(){return`
...
`,"<br />")}</p>
    ...
    `}}var Hr={VITE_SERVER_URL:"https://equal-rosemarie-deun-b63b6790.koyeb.app",VITE_BASE_URL:"/idle-intranet-service",BASE_URL:"/idle-intranet-service/",MODE:"production",DEV:!1,PROD:!0,SSR:!1};const{VITE_SERVER_URL:Fr}=Hr,ut=Fr,qr=async(t,e)=>{try{const n=await fetch(`${ut}/api/members/${t}?max=${e}`,{method:"GET"});if(!n.ok)throw new Error(`Failed to load data: ${n.statusText}`);const s=await n.json();return[s.data,s.total]}catch(n){throw console.error("Error loading page:",n),n}},Vr=async(t,e,n)=>{try{const s=encodeURIComponent(t),r=await fetch(`${ut}/api/members/search/${s}?max=${e}&page=${n}`,{method:"GET"});if(!r.ok){const o=await r.json();throw new Error(`Failed to search members: ${o.error}`)}const i=await r.json();return[i.data,i.total]}catch(s){throw console.error("Error searching members:",s.message),s}},jr=async(t,e=!1,n=!1)=>{try{const s=await fetch(`${ut}/api/member/${t}?isAdmin=${e}&isOwner=${n}`,{method:"GET"});if(!s.ok)throw new Error(`Failed to load data: ${s.statusText}`);return(await s.json()).data}catch(s){throw console.error("Error loading member data:",s),s}},Wr="/idle-intranet-service/assets/no-result-CZkYnNXR.png";class zr extends z{constructor(){super("#main");T(this,"handlePageChange",n=>{this.currentPage=n;const s=document.getElementById("search-input").value;s?this.searchMember(s):this.getMember()});T(this,"handleFormSubmit",n=>{n.preventDefault(),this.currentPage=1;const s=document.getElementById("search-input").value;this.searchMember(s)});T(this,"searchMember",n=>{n===""?this.getMember():Vr(n,this.maxProfile,this.currentPage).then(([s,r])=>{this.contents=s,this.total=r,this.renderPagination(),this.renderTable(),this.updateTotalCount()}).catch(s=>{console.error("Failed to search members:",s.message)})});T(this,"getMember",()=>{qr(this.currentPage,this.maxProfile).then(([n,s])=>{this.contents=n,this.total=s,this.renderPagination(),this.renderTable(),this.updateTotalCount()}).catch(n=>{console.error("Failed to load page data:",n)})});T(this,"resetSearch",()=>{this.currentPage=1,document.getElementById("search-input").value="",this.getMember()});T(this,"updateTotalCount",()=>{document.querySelectorAll(".members-container em").forEach(n=>{n.innerText=this.total})});T(this,"renderPagination",()=>{this.pagination=new Ur({currentPage:this.currentPage,maxPage:Math.ceil(this.total/this.maxProfile),onPageChange:this.handlePageChange}),window.location.pathname===I.MEMBERS&&(document.body.querySelector(".pagination-container").innerHTML=this.pagination.html(),this.pagination.render())});T(this,"renderTable",()=>{const n=this.contents.map(s=>(this.Avatar=new Fe({url:s.profileImage}),[this.createMemberLink(`${this.Avatar.html()}<div>${s.name}</div>`,s.employeeNumber),this.createMemberLink(s.position,s.employeeNumber),this.createMemberLink(s.departmentName,s.employeeNumber),this.createMemberLink(s.email,s.employeeNumber),this.createMemberLink(s.phoneNumber,s.employeeNumber)]));if(this.ids=this.contents.map(s=>[s.employeeNumber]),this.table=new lt({headers:["이름","직무","조직","이메일","연락처"],contents:n}),this.total===0){const s=document.getElementById("search-input").value;document.getElementById("table-container").innerHTML=`
      ...
      `}else if(window.location.pathname===I.MEMBERS){const s=document.getElementById("table-container");s.innerHTML=this.table.html()}});this.Title=new $e({title:B.MEMBERS}),this.magnifyGlass=new U({svg:cr,options:{size:"18px",color:Y.DARK_GRAY}}),this.input=new ne({placeholder:"이름을 입력하세요",id:"search-input"}),this.currentPage=1,this.maxProfile=7,this.contents=[],this.total=0,this.ids=[],he().then(n=>{n&&this.getMember()})}createMemberLink(n,s){return`<a href="${I.MEMBERS}/${s}">${n}</a>`}render(){this.$container.innerHTML=`
    ...
    `}}const tt=[{icon:"🌴",type:"연차",days:"17일"},{icon:"💼",type:"반차",days:"3시간"},{icon:"🤰🏻",type:"출산 휴가",days:"90일"},{icon:"🫃🏻",type:"배우자 출산 휴가",days:"30일"},{icon:"🎀",type:"여성 휴가",days:"1일"},{icon:"👥",type:"가족 돌봄 휴가",days:"1일"},{icon:"👤",type:"기타 휴가",days:"1일"}],Yr=["전체 휴가","연차","반차","출산 휴가","배우자 출산 휴가","여성 휴가","가족 돌봄 휴가","기타 휴가"];function Zr(t){const e=[];for(let n=1;n<=t;n+=1)e.push(`${n}일`);return e}function Xr(t,e){const n=new Date(t);n.setDate(n.getDate()+e-1);const s=n.getFullYear(),r=String(n.getMonth()+1).padStart(2,"0"),i=String(n.getDate()).padStart(2,"0");return`${s}-${r}-${i}`}const j=class j{constructor({contents:e,onSelect:n,excludeFirst:s=!1,roundedBorder:r=!1,small:i=!1}){T(this,"toggleDropdown",()=>{j.openDropdown&&j.openDropdown!==this&&j.openDropdown.closeDropdown(),this.dropdownVisible=!this.dropdownVisible;const e=this.dropdownButton.nextElementSibling;e.style.display=this.dropdownVisible?"block":"none",this.dropdownButton.classList.toggle("dropdown-open",this.dropdownVisible),this.dropdownVisible?j.openDropdown=this:j.openDropdown=null});T(this,"closeDropdown",()=>{this.dropdownVisible=!1;const e=this.dropdownButton.nextElementSibling;e.style.display="none",this.dropdownButton.classList.remove("dropdown-open"),j.openDropdown=null});T(this,"updateSelection",e=>{this.onSelect(e),this.dropdownButton.innerHTML=`${e} ${this.chevronDown.html()}`,this.closeDropdown()});if(typeof n!="function")throw new Error("onSelect callback must be provided and must be a function");this.id=`select-${j.instanceCounter}`,j.instanceCounter+=1,this.contents=e||[],this.onSelect=n,this.dropdownVisible=!1,this.roundedBorder=r,this.small=i,this.chevronDown=new U({svg:lr,options:{size:"18px",color:Y.DARK_GRAY}}),this.filteredContents=s?e.slice(1):e,this.selectedItem=this.contents[0]||""}html(){const e=this.roundedBorder?"dropdown-btn rounded-border":"dropdown-btn underlined";return`
    ...
    `,this.Form.setEventListeners()}}class ci extends z{constructor(){super("#main"),this.store=H,this.Title=new $e({title:B.MEMBERS,desktopOnly:!0}),this.store=H}async setUserInfo(){if(!await he())return;this.user||(this.user=await this.store.getUser());const n=window.location.pathname.split("/");this.employeeNumber=n[n.length-1];const s=this.user.employeeNumber===this.employeeNumber;try{this.member=await jr(this.employeeNumber,this.user.isAdmin,s)}catch(r){console.error("Failed to fetch user data:",r)}this.PersonalDetails=new dn,this.ProfileInfo=new et({member:this.member,isWorking:this.user.employeeNumber===this.member.employeeNumber?await this.store.getUserIsWorking():!0})}async render(){await this.setUserInfo(),this.$container.innerHTML=`
      ...
      ${this.PersonalDetails.html()}
    `,this.PersonalDetails.render(this.member),this.ProfileInfo.render()}}const li=(t,e)=>{if(!e)return null;let n=null;return Object.keys(e).forEach(s=>{if(n)return;const r=e[s],i=[],o=s.replace(/:[^\s/]+/g,u=>(i.push(u.slice(1)),"([^\\/]+)")),a=new RegExp(`^${o}$`);t.match(a)&&(n=r)}),n};class ui{constructor(){T(this,"handleNavigatePage",e=>{e.preventDefault();const n=e.target.closest("a");n&&n.href&&(window.history.pushState(null,null,n.href),this.route(),this.activeNavBar())});this.title="CubeIT ",this.currentPage=null}setRoutes(){this.routes={[I.HOME]:{title:B.HOME,page:new Br},[I.ANNOUNCEMENT]:{title:B.ANNOUNCEMENT,page:new fr},[I.MEMBER]:{title:B.MEMBERS,page:new ci},[I.MEMBERS]:{title:B.MEMBERS,page:new zr},[I.PROFILE]:{title:B.PROFILE,page:new Kr},[I.WORK_MANAGE]:{title:B.WORK_MANAGE,page:new ri},[I.SIGNIN]:{title:B.SIGNIN,page:new ai}}}activeNavBar(){const e=window.location.pathname;this.Menu.active=e}route(){const e=window.location.pathname;he().then(s=>{!s&&e!==I.SIGNIN&&(window.history.pushState(null,null,I.SIGNIN),this.routes[I.SIGNIN].page.render())});const n=li(e,this.routes);this.currentPage&&this.currentPage.cleanUp&&this.currentPage.cleanUp(),n&&n.page?(this.currentPage=n.page,document.title=this.title+n.title,this.currentPage.render()):this.notFoundPage.render(),this.activeNavBar()}init(){window.addEventListener("popstate",()=>this.route()),document.body.addEventListener("click",this.handleNavigatePage),this.Menu=H.Menu,this.notFoundPage=new wr,this.setRoutes(),this.route()}}class di extends z{constructor(){super(".header-container"),this.store=H,this.icon=new U({svg:hr,options:{color:Y.DARKEST_GRAY,size:"14px"}}),this.Button=new ue({variant:"ghost",content:`${this.icon.html()} 로그아웃`,size:"small"})}async renderAvatar(){if(!await he())return;this.user||(this.user=await this.store.getUser(),this.Avatar=new Fe({url:this.user.profileImage}));const n=document.querySelector("header .avatar-container");n.innerHTML=this.Avatar.html()}render(){this.$container.innerHTML=`
      <div class="wrapper header-items">
        <div class="avatar-container"></div>
        <div class="logout-btn-wrapper">${this.Button.html()}</div>
      </div>
    `,document.querySelector(".logout-btn-wrapper button").addEventListener("click",Ct),this.renderAvatar()}}class hi extends z{constructor(){super(".navbar")}render(){this.$container.innerHTML=`
      ...
      <ul class="menu-list"></ul>
    `}}const mi=[{path:I.HOME,title:B.HOME,icon:sr},{path:I.MEMBERS,title:B.MEMBERS,icon:rr},{path:I.PROFILE,title:B.PROFILE,icon:ir},{path:I.WORK_MANAGE,title:B.WORK_MANAGE,icon:sn}];class pi extends z{constructor(){super(".menu-list"),this.menus=mi,this._active=I.HOME}set active(e){this._active=e,this.render()}isActiveMenu(e){return e===I.HOME&&this._active===I.HOME?!0:e===I.HOME?!1:this._active.startsWith(e)}render(){this.$container.innerHTML=this.menus.map(e=>{const n=this.isActiveMenu(e.path),s=new U({svg:e.icon,options:{color:n?Y.PRIMARY:Y.BLACK}});return`
          <li>
        ...
        `}).join("")}}class fi extends z{constructor(){super("#app"),this.store=H}renderHeader(){this.Header=new di,this.Header.render()}renderNavbar(){this.NavBar=new hi,this.NavBar.render(),this.Menu=new pi,this.Menu.render(),this.store.setMenu(this.Menu)}render(){this.$container.innerHTML=`
      <header class="header-container desktop-only"></header>
      <nav class="navbar"></nav>
      <main id="main" class="main-container"></main>
    `,this.renderHeader(),this.renderNavbar()}}class gi{constructor(){this.Layout=new fi,this.Route=new ui,this.init()}init(){this.Layout.render(),this.Route.init()}}document.addEventListener("DOMContentLoaded",new gi);
